...
//            }
    ...
    public PageInfo<AdvertisementPo> findAllAdByTimeId(String beginDate, String endDate, Long timeId, Integer page, Integer pagesize)
    {
        ...
        if(endDate!=null)
        {
            criteria.andEndDateLessThanOrEqualTo(LocalDate.parse(endDate, DateTimeFormatter.ofPattern("yyyy-MM-dd")));
        }
...
//        }
     ...
     * @author ChengYang Li
     */
    public ReturnObject updateAdvertisementById(Long adId, AdvertisementVo vo)
    {
        ReturnObject<Object> retObj;
        AdvertisementPo po=advertisementPoMapper.selectByPrimaryKey(adId);
        //不修改不存在的广告
        ...
        {
            po.setWeight(Integer.valueOf(vo.getWeight()));
        }
        ...
        {
            LocalDate beginDate =
                    LocalDate.parse(vo.getBeginDate(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            if(beginDate.isAfter(po.getEndDate()))
            {
        ...
        {
            LocalDate endDate =
                    LocalDate.parse(vo.getEndDate(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            if(endDate.isBefore(po.getBeginDate()))
            {
                return new ReturnObject<>(ResponseCode.FIELD_NOTVALID);
            }
        ...
        {
            if(vo.getRepeat()=="true")
            {
                po.setRepeats((byte)1);
            }
            if(vo.getRepeat()=="false")
            {
                po.setRepeats((byte)0);
            }
        }
        po.setGmtModified(LocalDateTime.now());
        po.setState((byte)0);
