...
public class CustomerDao {
    ...
    public ReturnObject<Customer> createCustomer(Customer customer) {
        ...
        customerPo.setGmtModified(null);
        //check in user table
        ...
        try {
            ...
            if (ret == 0) {
                //插入失败
                logger.debug("insertCustomer: insert Customer fail " + customerPo.toString());
                returnObject = new ReturnObject<>(ResponseCode.RESOURCE_ID_NOTEXIST, String.format("新增失败：" + customerPo.getUserName()));
            } else {
                //插入成功
                logger.debug("insertCustomer: insert customer = " + customerPo.toString());
    ...
    public Integer useRebate(Long customerId, Integer num) {
        ...
        if(customerPo==null)return 0;
        if(customerPo.getPoint()<num)return 0;
        customerPo.setPoint(customerPo.getPoint()-num);
        return num;
    }
}
